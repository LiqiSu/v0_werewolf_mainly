...
- TOO obvious or firm alliance or accusation is suspicious WITHOUT roleclaims (since only werewolves naturally know teammates and enemies).
- So true-claim and fake-claim are BOTH STRATEGIES.
- Consistency and Coherence ARE important evidence.
...
Night deaths resolve {(f"({'/'.join(save)} can prevent wolf kill)") if save else ""}.
...
NO environmental complexity, NO physical movement, NO sight perception.
...
Output:
- Character's private internal reasoning.
"""
    return rules.strip()
...
IMPORTANT:
- Output ONLY important strategic items
- ONE item per line
- Do NOT force; No more ideas = NO MORE ITEMS
- Be SHORT AND COMPRESSED
- MAX 10 words per item

-----
PRIVATE REASONING ITEM FORMAT (MAX 3 items):
intention|reasoning

Example:
shift suspicion from self|protect self

-----
SPEECH ITEM FORMAT (MAX 3 items):
...
Example:
roleclaim self seer|inspected AA last night, village

-----
*Manipulation, misleading and deception in messages are often needed if intentions are not straightforward (eg. hide, subtle)
...
-----
PRIVATE REASONING ITEM FORMAT (MAX 1 item):
intention|reasoning

IMPORTANT:
- ONLY important strategic items
- ONE item per line
- Do NOT force; No more ideas = NO MORE ITEMS
- Be SHORT AND COMPRESSED
- MAX 10 words per item
...
def day_phase():
    ...
    for discuss_round in [1, 2]:
        for speaker in speak_order:
            ...
            payload = build_payload_speak(
                ...
                reminder="You're discussing who to eliminate today. BE CAUTIOUS. DO NOT TRUST EASILY OR ACCUSE HASTILY. DO NOT mix speech with internal reasoning. EVERYONE CAN HEAR YOUR SPEECH. Help your TEAM WIN the game."
            )
            model=chars["characters"][speaker]["llm_model"]
    ...
    for voter in speak_order:
        ...
        payload = build_payload_answer(
            ...
            valid_options=valid_options,
            reminder="Help your TEAM WIN the game. Evaluate risk and consider carefully. You can abstain or try to force a tie if it's strategically better. *Majority's consensus does NOT necessarily mean truth; analyze independently."
        )
