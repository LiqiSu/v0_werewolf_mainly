...
def convert_output_to_events(char_name, output):
    ...
    for a in output.get("actions", []):
        ...
        etype = "action"
        if content.startswith("wolfchat"):
            etype = "wolfchat"
        events.append({
            "type": etype,
            "actor": char_name,
...
def build_perception(events, scene):
    ...
    for event in events:
        ...
        # WEREWOLF FILTERING
        # ------------------------
        event_type = event.get("content", "").lower()
        is_wolfchat = "wolfchat" in event_type or event["type"] == "wolfchat"
        is_seer_check = "check" in event_type and chars_all["characters"].get(actor, {}).get("game_status", {}).get("role") == "seer"
        is_death_announce = "died" in event_type or "death" in event_type
        ...
        if event["type"] != "thought":
            for t in targets:
                ...
                # Filter: if wolfchat, only werewolves see
                char_role = chars_all["characters"].get(t, {}).get("game_status", {}).get("role", "")
                char_faction = chars_all["characters"].get(t, {}).get("game_status", {}).get("faction", "")
                if is_wolfchat and char_faction != "werewolves":
                    continue
                if is_seer_check and t != actor:
                    continue  # Only seer sees check
                perception[t][eid] = {
                    "event": event,
                    "confidence": 100
                }
        ...
        if event["type"] != "thought":
            for c in scene_chars:
                ...
                # Filter for scene
                char_role = chars_all["characters"].get(c, {}).get("game_status", {}).get("role", "")
                char_faction = chars_all["characters"].get(c, {}).get("game_status", {}).get("faction", "")
                if is_wolfchat and char_faction != "werewolves":
                    continue
                if is_seer_check and c != actor:
                    continue
                perception[c][eid] = {
                    "event": event,
                    "confidence": 100
...
def character_decision_and_action(character_name, perceived_events):
    ...
    # -------------------------
    prompt = (
        "You are a character in Werewolves of Miller's Hollow.\n\n"
        "You MUST act ONLY based on:\n"
        "- the character sheet\n"
        "- the perceived events\n\n"
        "CRITICAL RULES:\n"
        "1. You DO NOT know anything outside the perception.\n"
        "2. You DO NOT know other characters' thoughts or roles.\n"
        ...
        "WEREWOLVES RULES:\n"
        "- Day: Debate (speech <100 chars), then vote 'vote for [name]'.\n"
        "- Night: Depending on role, 'kill [name]', 'check [name]', 'protect [name]', 'save [name]', 'poison [name]'.\n"
        "- Werewolves can 'wolfchat [message <100 chars]' to communicate privately.\n"
        "- You know the rules from your knowledge.\n\n"
...
def assign_werewolf_roles(chars):
    """
    Randomly assign Werewolves roles to characters.
    Roles: 4 werewolves, 5 villagers, 1 witch, 1 seer, 1 hunter, 1 knight, 1 guardian angel
    """
    roles = (
        ["werewolf"] * 4 +
        ["villager"] * 5 +
        ["witch", "seer", "hunter", "knight", "guardian angel"]
    )
    random.shuffle(roles)
    
    char_names = list(chars["characters"].keys())
    if len(char_names) != 14:
        raise ValueError("Expected 14 characters, got {}".format(len(char_names)))
    
    for i, name in enumerate(char_names):
        role = roles[i]
        faction = "werewolves" if role == "werewolf" else "villagers"
        chars["characters"][name]["game_status"]["role"] = role
        chars["characters"][name]["game_status"]["faction"] = faction

def preset_werewolf_knowledge(chars):
    """
    Preset knowledge: each knows others' names, full game rules, scene is table.
    """
    char_names = list(chars["characters"].keys())
    rules = """
    Werewolves of Miller's Hollow Rules:
    - 14 players: 4 werewolves, 5 villagers, 1 witch, 1 seer, 1 hunter, 1 knight, 1 guardian angel.
    - Day: Debate (2 rounds, 100 char limit), then vote to lynch. Tie = no lynch.
    - Wolf reveal or knight challenge ends day early.
    - Night: Seer checks, werewolves discuss+vote kill, guardian protects, witch saves/poison, hunter shoots if dead.
    - Deaths announced morning, no roles.
    - Win: Werewolves if all villagers dead, villagers if all werewolves dead.
    """
    ...
    for name in char_names:
        ...
        for other in char_names:
            if other != name:
                if other not in char["knowledge"]["characters"]:
                    char["knowledge"]["characters"][other] = {
                        "basics": {},
                        "role": {},
                        "traits": {},
                        "background": {},
                ...
                char["knowledge"]["characters"][other]["basics"]["name"] = {"value": other, "confidence": 100}
        
        # Add rules
        char["knowledge"]["hard_knowledge"]["werewolf_rules"] = {
            "description": rules,
            "confidence": 100,
            "decay": "slow"
        }
        ...
        # Scene is table
        char["knowledge"]["scene"]["location"] = {"value": "table", "confidence": 100}

def init_werewolf_game():
    """
    Initialize the Werewolves game: assign roles, preset knowledge.
    """
    chars = load_json(CHARACTERS_FILE, {"characters": {}})
    assign_werewolf_roles(chars)
    preset_werewolf_knowledge(chars)
    save_json(CHARACTERS_FILE, chars)

def advance_phase(game_rounds):
    """
    Advance to next phase.
    """
    if game_rounds["phase"] == "day":
        game_rounds["phase"] = "night"
        game_rounds["night_count"] += 1
    elif game_rounds["phase"] == "night":
        game_rounds["phase"] = "day"
        game_rounds["day_count"] += 1
        game_rounds["current_round"] += 1
    game_rounds["phase_start_time"] = 0  # dummy

def execute_day_phase(chars, game_rounds, events):
    """
    Handle day phase: debate, vote.
    For now, simple: collect speeches and votes from events.
    """
    # Placeholder: process events for speeches and votes
    speeches = []
    votes = {}
    for event in events:
        if event["type"] == "speech":
            speeches.append({"actor": event["actor"], "content": event["content"]})
        elif event["type"] == "action" and "vote" in event["content"]:
            # Parse vote
            target = event["content"].replace("vote for ", "").strip()
            votes[event["actor"]] = target
    
    # If 2 rounds, but for simplicity, assume one round for now
    if len(speeches) >= len([c for c in chars["characters"].values() if c["game_status"]["death_count"] == 0]):
        # Proceed to vote
        game_rounds["voting_tracker"] = votes
        # Resolve vote
        vote_counts = {}
        ...
        if len(candidates) == 1:
            ...
            chars["characters"][lynched]["game_status"]["death_count"] += 1
            chars["characters"][lynched]["game_status"]["was_lynched"] = True
            game_rounds["eliminated_players"].append(lynched)
        # Else tie, no lynch
        advance_phase(game_rounds)

def execute_night_phase(chars, game_rounds, events):
    """
    Handle night phase: actions.
    """
    # Placeholder: process events for actions
    actions = {}
    for event in events:
        if event["type"] == "action":
            actions[event["actor"]] = event["content"]
    
    # Sequence: seer, wolves, guardian, witch, resolve, hunter
    ...
    for actor, action in actions.items():
        if "kill" in action:
            kill_target = action.replace("kill ", "").strip()
        elif "protect" in action:
            protect_target = action.replace("protect ", "").strip()
        elif "save" in action:
            save_target = action.replace("save ", "").strip()
        elif "poison" in action:
            poison_target = action.replace("poison ", "").strip()
    
    deaths = []
    if kill_target and kill_target != protect_target:
        chars["characters"][kill_target]["game_status"]["death_count"] += 1
        deaths.append(kill_target)
    if poison_target:
        chars["characters"][poison_target]["game_status"]["death_count"] += 1
        deaths.append(poison_target)
    
    # Hunter if dead
    for d in deaths:
        if chars["characters"][d]["game_status"]["role"] == "hunter":
            # Hunter shoots, but for now, assume shoots someone
            pass
    
    game_rounds["kill_targets"] = deaths
    advance_phase(game_rounds)

def check_win_conditions(chars, game_rounds):
    """
    Check if game is over.
    """
    alive = [c for c in chars["characters"].values() if c["game_status"]["death_count"] == 0]
    werewolves = [c for c in alive if c["game_status"]["faction"] == "werewolves"]
    villagers = [c for c in alive if c["game_status"]["faction"] == "villagers"]
    
    if not werewolves:
        game_rounds["game_status"] = "villagers_won"
    elif len(werewolves) >= len(villagers):
        game_rounds["game_status"] = "werewolves_won"
...
def main():
    ...
    # --- Player Participation Setup ---
    print("Welcome to Werewolves of Miller's Hollow!")
    participate = input("Do you want to play as a character? (yes/no): ").strip().lower()
    player_character = None
    player_role = None
    fixed_characters = ["AA", "CO", "DO", "FO", "GA", "LO", "MA", "NO", "PA", "SA", "TO", "VA", "WA", "YA"]
    roles_list = ["werewolf"] * 4 + ["villager"] * 5 + ["witch", "seer", "hunter", "knight", "guardian angel"]

    if participate == "yes":
        print("Available characters:", ", ".join(fixed_characters))
        while True:
            pc = input("Pick your character (by code): ").strip().upper()
            if pc in fixed_characters:
                player_character = pc
                break
            print("Invalid character. Try again.")
        print("Available roles:", ", ".join(set(roles_list)))
        pr = input("Pick your role (or type 'random'): ").strip().lower()
        if pr in set(roles_list):
            player_role = pr
        else:
            player_role = None  # random

    # --- Random Trait/Role Assignment ---
    rand_traits = input("Randomly generate character traits? (yes/no): ").strip().lower() == "yes"

    # --- Wait for explicit game start ---
    while True:
        start = input("Type 'start the game' to begin: ").strip().lower()
    ...
    for name in fixed_characters:
        ...
        char["gender"] = random.choice(["male", "female"]) if rand_traits else ""
        char["age_apparent"] = random.randint(18, 25) if rand_traits else None
        char["age_actual"] = char["age_apparent"]
        char["species"] = "human"
        char["location"] = "table"
        char["activity"] = "sitting"
        chars["characters"][name] = char

    # --- Assign Roles ---
    assigned_roles = roles_list.copy()
    random.shuffle(assigned_roles)
    for i, name in enumerate(fixed_characters):
        if participate == "yes" and name == player_character:
            if player_role and player_role in assigned_roles:
                role = player_role
                assigned_roles.remove(role)
            else:
                role = assigned_roles.pop(0)
            faction = "werewolves" if role == "werewolf" else "villagers"
            chars["characters"][name]["game_status"]["role"] = role
            chars["characters"][name]["game_status"]["faction"] = faction
        else:
    ...
    scene["location"]["key"] = "table"
    scene["location"]["name"] = "Table at Miller's Hollow"
    scene["location"]["description"] = "A large round table where all villagers gather."
    scene["time"]["time_of_day"] = "night"
    scene["time"]["elapsed_hours"] = 0.0
    scene["characters_present"] = {name: {"focus": 100} for name in fixed_characters}
    ...
    # --- Show Player Role ---
    if participate == "yes":
        print(f"You are playing as {player_character}. Your role is: {chars['characters'][player_character]['game_status']['role'].upper()}.")

    # --- Game Rounds File ---
    game_rounds = {
        "current_round": 1,
        "phase": "night",  # always start with night
        "round_start_time": 0,
        "phase_start_time": 0,
        "day_count": 0,
        "night_count": 1,
    ...
    while True:
        ...
        scene = load_json(SCENE_STATE_FILE, scene_template())
        game_rounds = load_json(GAME_ROUNDS_FILE, game_rounds)
        ...
        for char_name, perceived in perception.items():
            if char_name not in chars:
                continue
            # Player control
            if participate == "yes" and char_name == player_character:
                print(f"It's your turn, {player_character}!")
                print("Perceived events:")
                for eid, pdata in perceived.items():
                    if eid == "__scene__":
                        continue
                    e = pdata.get("event", {})
                    print(f"  [{e.get('type','')}] {e.get('actor','')}: {e.get('content','')}")
                action = input("What do you say or do? (speech, vote, wolfchat, etc): ").strip()
                # For now, treat as speech
                output = {
                    "thoughts": [],
                    "decisions": [],
                    "actions": [],
                    "speech": [
                        {"target": [], "content": action, "intensity": 50}
                    ]
                }
                all_character_outputs[char_name] = output
            else:
        ...
        # 6. WEREWOLF PHASE LOGIC
        if game_rounds["phase"] == "day":
            execute_day_phase(chars, game_rounds, new_events)
        elif game_rounds["phase"] == "night":
            execute_night_phase(chars, game_rounds, new_events)
        check_win_conditions(chars, game_rounds)
        save_json(GAME_ROUNDS_FILE, game_rounds)
        save_json(CHARACTERS_FILE, chars)

        # 7. UPDATE CHARACTERS FROM PERCEPTION
        for char_name, perceived in perception.items():
            update_character_from_perception(char_name, perceived)

        # 8. End condition
        if game_rounds["game_status"] != "in_progress":
            print(f"Game over! Result: {game_rounds['game_status']}")
            break
