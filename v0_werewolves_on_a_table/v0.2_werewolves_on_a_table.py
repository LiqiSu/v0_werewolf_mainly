...
def parse_llm_output(raw_text, mode="speak", valid_options=None,
    ...
    """
    Modes:
    speak   -> internal_reasoning + speech
    answer  -> internal_reasoning + answer
    summary -> summary
    signals -> signals   (multi-line list OR DROP)
    ...
    def normalize_label(label):
        ...
        s = re.sub(r'[^a-z_ ]', '', s)
        s = re.sub(r'\s+', ' ', s)
        for canon, arr in key_alias.items():
            for a in arr:
                if s == a:
                    return canon
        return None
    ...
    def normalize_signals(v):
        ...
        # split by newline / comma / semicolon / pipe / period
        parts = re.split(r'[\n;,|.]+', v)
    ...
    if mode == "signals" and result.get("signals", "").strip():
        return result
    ...
    if mode == "signals":
        ...
        for i, line in enumerate(lines):
            ...
            for j in range(i + 1, len(lines)):
                ...
                # stop if another label begins
                if re.match(r'^[A-Za-z_ ]{2,40}\s*:', nxt):
                    break

                collected.append(nxt)

            val = normalize_signals("\n".join(collected))
            if val:
                return {"signals": val}
...
def game_rule(player_number, roles):
    ...
    if has("werewolf") or has("werewolves"):
        mech.append("Werewolves: private discuss then vote 1 night kill target; tie=no kill.")
    ...
    if has("witch"):
        mech.append("Witch: 2 potions in the entire game; 1 save potion cancels 1 wolf kill (witch can KNOW wolf's target if save potion's still in possession), 1 poison kills 1 player.")
...
- EVERYONE MAY LIE; EVERYONE CAN LIE; FAKE CLAIMS AND TRUE CLAIMS ARE BOTH STRATEGIES.
...
def system_prompt_rules():
    rules = """
You are simulating a character in a fully abstract structured social deduction game.
...
- MAY use memory; memory reliability: announcement, knowledge > internal_reasoning, action > speech.
- DO NOT overthink. Information is limited, uncertainty is natural.
- Output MUST be compressed, obeying WORD LIMITS.
- NO filler, repetition, hedging, or empty statements.

Output:
- Internal reasoning summary of the character, MAX 20 words.
"""
    return rules.strip()
...
def call_llm(actor_name, model_name, sys_prompt, user_payload, 
    ...
    if call_type == "speech":
        ...
        speech = " ".join(data.get("speech", default_speech).split()[:max_len])
        if speech != default_speech:
            internal_reasoning = " ".join(data.get("internal_reasoning", "").split()[:max_len]).strip()
        else:
            internal_reasoning = ""
...
ALL_ROLES = ["werewolf", "villager", "seer", "witch", "hunter", "knight", "guardian angel"]

# --------------------------------------------------
# detect names and roles from raw text
# --------------------------------------------------
def detect_names(text):
    ...
    for name in ALL_CHARACTER_NAMES:
        if re.search(rf"\b{name}\b", upper):
            found.add(name)
        elif re.search(rf"\b{name}'S\b", upper):
            found.add(name)
...
def detect_roles(text):
    ...
    roles = ALL_ROLES + ["wolf", "werewolves", "wolves", "villagers", "village", "guardian", "angel"]
    for role in roles:
        if re.search(rf"\b{role}\b", lower):
            found.add(role)
        elif re.search(rf"\b{role}'S\b", lower):
            found.add(role)

    return sorted(found)

# --------------------------------------------------
# normalize signal line
# --------------------------------------------------
def normalize_signal_line(line):
    if not line:
        return None
    ...
    # maybe will output "suspect: AA" form - parse? colon?
    parts = s.split()
    if len(parts) < 1:
        return None
    ...
    allowed_1 = {"wait", "unsure", "postpone"}

    allowed_2 = {
        "suspect", "distrust", "oppose", "accuse", "kill", "eliminate",
        "defend", "trust", "ally", "save", "agree_with", "alliance",
        "observe"
    }
    ...
    if action in allowed_1:
        return action.upper()

    if action in allowed_2 and len(parts) >= 2:
        target = parts[1].upper()
        if target in ALL_CHARACTER_NAMES:
            return f"{action} {target}"

    if action in allowed_3 and len(parts) >= 3:
        target = parts[1].upper()
        role = parts[2].lower()

        if target in ALL_CHARACTER_NAMES:
            return f"{action} {target} {role}"
...
# main extractor
# --------------------------------------------------
def extract_signals_with_llm(text, speaker, model_pool=None):
    """
    Returns list[str]
    Example:
    ["suspect AA", "vote AA"]
    ...
    if speaker not in ALL_CHARACTER_NAMES:
        speaker = "UNKNOWN"

    names_found = detect_names(text)
    visible_names = sorted(set(names_found + [speaker]))
    roles_found = detect_roles(text)
    visible_roles = sorted(set(roles_found))

    prompt = f"""
Extract strategic signals from a social deduction game.
TAKE FACE-VALUE ONLY. DO NOT OVER-INTERPRET. DO NOT ASSUME OR INFER ANYTHING BEYOND THE PROVIDED TEXT.
DROP WHEN NOTHING IS EXPLICITLY STATED.

Speaker is {speaker}.
Words I / me / my / myself refer to {speaker}.
the ONLY VALID NAMES are: {", ".join(visible_names)}.
the ONLY VALID ROLES are: {", ".join(visible_roles)}.
In addition:
"inspected" (NOT "investigate") mentioned = ROLE "seer" involed.
"protected" mentioned = ROLE "guardian_angel" involved.
"save/poison/potion" mentioned = ROLE "witch" involved.

ALLOWED OUTPUTS (one per line only):
distrust NAME
oppose NAME
suspect NAME
accuse NAME
...
In addition:
"target NAME" mentioned = "eliminate NAME"
"vote NAME" mentioned = "eliminate NAME"

Rules:
- ONLY the specified OUTPUTS, NAMES, and ROLES are ALLOWED
- MAX 3 signals; 3 lines
- No explanation
- No punctuation
- If no useful signal: DROP

Return EXACTLY this format:

signals:
<<<
signals here, or "DROP" if discard
>>>
...
"""
    ...
    try:
        response = client.chat.completions.create(
            model=random.choice(model_pool or MODEL_SMALL),
            messages=[
        ...
        data = parse_llm_output(raw_text=raw_content, mode="signals")
        raw = data.get("signals", "DROP")

        log_llm_interaction(speaker, "extract_signals_with_llm", text, raw_content, raw, "llm_filter")

        if not raw:
            return []
        if raw.upper().strip() == "DROP":
            return []
        ...
        seen = set()
        for line in lines:
            sig = normalize_signal_line(line)
...
def route_self_bucket(bucket, slot, content):
    ...
    # -----------------------------
    if speech:
        signals = extract_signals_with_llm(speech, performer)
        if signals:
...
def route_other_bucket(bucket, slot, content):
    ...
    signals = extract_signals_with_llm(speech, performer) if speech else []
...
def day_phase():
    ...
    for discuss_round in [1, 2]:
        for speaker in speak_order:
            ...
            payload = build_payload_speak(
                ...
                reminder="You're discussing who to eliminate today. Role-claims and evidence are IMPORTANT. Consider carefully IF to let others know your private night actions. EVERYONE CAN HEAR YOUR SPEECH here. Help your TEAM WIN the game."
            )
            model=chars["characters"][speaker]["llm_model"]
    ...
    for voter in speak_order:
        ...
        payload = build_payload_answer(
            ...
            valid_options=valid_options,
            reminder="Help your TEAM WIN the game. Evaluate risk and consider carefully. You can abstain or try to force a tie too if it's strategically better."
        )
